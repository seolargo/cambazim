...
const ActivateAccount = ({ router }) => {
    ...
    const showLoading = () => (loading ? <h2>Yükleniyor...</h2> : '');

    return (
        <Layout>
            <div className="container">
                <h3 className="pb-4">Selam {name}, hesabını aktifleştirmek ister misin?</h3>
                {showLoading()}
                {error && error}
                {success && 'Hesabını başarılı bir şekilde aktifleştirdin. Lütfen giriş yap.'}
                {showButton && (
